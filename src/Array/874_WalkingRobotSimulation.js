...
var robotSim = function (commands, obstacles) {
  const obstacleLookup = obstacles.reduce((blocks, [x, y]) => {
    blocks.add(`${x},${y}`);
    return blocks;
  }, new Set());
  ...
  commands.forEach((command) => {
    if (command === -2) {
      currDirection = currDirection - 1 + 4 * (currDirection === 0);
    } else if (command === -1) {
      currDirection = (currDirection + 1) % 4;
    } else {
      const [xDiff, yDiff] = directions[currDirection];
      let distance = command;
      while (distance && !obstacleLookup.has(`${positionX + xDiff},${positionY + yDiff}`)) {
        positionX += xDiff;
        positionY += yDiff;
        distance -= 1;
      }
      maxDistance = Math.max(maxDistance, positionX ** 2 + positionY ** 2);
    }
  });
