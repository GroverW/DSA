...
var getMaximumGold = function (grid) {
  ...
  const collectGold = (row, col, currentGold) => {
    const goldAmount = grid[row][col];
    ...
    for (let [rDiff, cDiff] of tests) {
      ...
      collectGold(rNext, cNext, nextGold)
    }

    grid[row][col] = goldAmount;
  }
...
for (let test of tests) {
  logOutList(getMaximumGold(test));
}