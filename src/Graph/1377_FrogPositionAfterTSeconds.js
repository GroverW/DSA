...
var frogPosition = function (n, edges, t, target) {
  ...
  while (queue.length) {
    ...
    for (let end = queue.length - 1; end >= 0; end -= 1) {
      ...
      const children = connections[curr].filter((next) => !chances[next]);
      const chance = 1 / children.length;

      if (curr === target && (elapsed === t || !children.length)) return chances[curr];

      for (let child of children) {
        chances[child] = chances[curr] * chance;
        queue.push(child);
      }
    }
