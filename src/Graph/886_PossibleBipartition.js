...
var possibleBipartition = function (N, dislikes) {
  ...
  const addToGroup = (person, groupA, groupB) => {
    if (groupB.has(person)) return false;
    if (groupA.has(person)) return true;

    groupA.add(person);

    for (let nextDislike of adjList[person]) {
      if (!addToGroup(nextDislike, groupB, groupA)) return false;
    }
  ...
  for (let i = 1; i <= N; i += 1) {
    if (group1.has(i) || group2.has(i)) continue;
    if (!addToGroup(i, group1, group2)) return false;
  }
  return true;
};
...
const tests = [
  ...
  [2, [[1, 2], [2, 1]]],
  [9, [[1, 2], [1, 3], [2, 4], [3, 4], [5, 6], [6, 7], [6, 8], [7, 9], [5, 8]]],
...
for (let test of tests) {
  logOutList(possibleBipartition(...test));
}